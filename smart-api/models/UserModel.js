...
class UserModel {
    ...
    getUserByEmail(email) {
        return new Promise((resolve, reject) => {
            connection.query('SELECT id_user, email, kata_sandi FROM table_user WHERE email = ?', [email], (err, results) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(results.length > 0 ? results : null);
    ...
    updateSandiUser(email, hashPass) {
        return new Promise((resolve, reject) => {
    ...
    getAdminByEmail(email) {
        return new Promise((resolve, reject) => {
            connection.query('SELECT id_admin, email, kata_sandi FROM table_admin WHERE email = ?', [email], (err, results) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(results.length > 0 ? results : null);
    ...
    updateSandiAdmin(email, hashPass) {
        return new Promise((resolve, reject) => {
